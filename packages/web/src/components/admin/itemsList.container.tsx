...
const ItemsList: React.FC<ItemsListProps> = ({
  ...
  const onDelete = (category: 'brand' | 'type' | 'device', id: number) => {
    if (category === 'brand') {
      removeBrand(id);
      deleteBrand(id);
    }
          ...
          <div key={type.id} className={`${styles.listItem}`}>
            ...
            <Tooltip title='Delete category' placement='right'>
              <a style={{ marginLeft: 'auto' }}>
                <DeleteOutlined onClick={() => onDelete('type', type.id)} />
              </a>
            </Tooltip>
          </div>
        ))}
        ...
        {brands.map((brand) => (
          <div key={brand.id} className={`${styles.listItem}`}>
            ...
            <Tooltip title='Delete brand' placement='right'>
              <a style={{ marginLeft: 'auto' }}>
                <DeleteOutlined onClick={() => onDelete('brand', brand.id)} />
              </a>
            </Tooltip>
          </div>
        ))}
        ...
        {devices.map((device) => (
          ...
          >
            ...
            <Tooltip title='Delete device' placement='right'>
              <a style={{ marginLeft: 'auto' }}>
                <DeleteOutlined onClick={() => onDelete('device', device.id)} />
              </a>
            </Tooltip>
          </div>
        ))}
